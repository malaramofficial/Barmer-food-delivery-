...
create table if not exists public.restaurants(id uuid primary key default gen_random_uuid(),owner_id uuid references auth.users(id) on delete set null,name text not null,cuisine text,address text not null,area text not null,phone text,rating numeric(2,1) default 0,delivery_fee numeric(10,2) default 0,is_open boolean default false,is_approved boolean not null default false,latitude double precision,longitude double precision,created_at timestamptz not null default now());
