...
Deno.serve(async (req) => {
  ...
  const { data: order, error } = await service.from('orders').insert({ customer_id: user.id, restaurant_id: restaurant.id, delivery_address: body.address, delivery_latitude: lat, delivery_longitude: lng, subtotal, delivery_fee: deliveryFee, total, payment_method: paymentMethod, status: 'restaurant_notified' }).select().single();
