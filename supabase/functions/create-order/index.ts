...
Deno.serve(async req=>{
 ...
 const body=await req.json().catch(()=>null); if(!body?.restaurant_id||!Array.isArray(body.items)||!body.items.length||!body.address)return json({error:'restaurant_id, items and address are required'},400);
 const ids=body.items.map((x:any)=>x.menu_item_id).filter(Boolean); if(!ids.length||ids.length>50)return json({error:'Invalid items'},400);
 ...
 const orderItems=body.items.map((x:any)=>{const item=byId.get(x.menu_item_id);const qty=Math.max(1,Math.min(20,Number(x.quantity)||1));subtotal+=Number(item.price)*qty;return{menu_item_id:item.id,item_name:item.name,unit_price:item.price,quantity:qty};});
 const deliveryFee=Number(restaurant.delivery_fee||0),total=subtotal+deliveryFee;
 ...
 if(itemError){await supabase.from('orders').delete().eq('id',order.id).eq('customer_id',user.id);return json({error:'Could not save order items'},500);}
