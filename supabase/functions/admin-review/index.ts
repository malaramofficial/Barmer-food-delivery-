...
Deno.serve(async req=>{
 ...
 const {data:application,error}=await service.from(table).update({status:body.status,reviewed_by:user.id,reviewed_at:new Date().toISOString(),review_note:body.note??null}).eq('id',body.application_id).eq('status','pending').select().single();
