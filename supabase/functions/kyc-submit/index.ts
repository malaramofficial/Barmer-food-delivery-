...
Deno.serve(async req => {
  ...
  const anon = createClient(base, anonKey);
  const sb = createClient(base, serviceKey);
  const { data: { user } } = await anon.auth.getUser(token);
  if (!user) return json({ error: 'Unauthorized' }, 401);

  const b = await req.json().catch(() => null);
  const type = String(b?.document_type || '').trim().toLowerCase();
  ...
  const leaf = path.slice(user.id.length + 1);
  const meta = (object || []).find((x: any) => x.name === leaf);
  if (!meta) return json({ error: 'Uploaded file not found' }, 400);
  const mime = String(meta.metadata?.mimetype || meta.metadata?.mimeType || '').toLowerCase();
  ...
  const { data, error } = await sb.from('kyc_documents').insert({
    user_id: user.id, document_type: type, application_type: applicationType,
    application_id: applicationId, storage_path: path, original_name: originalName,
    mime_type: mime || null, size_bytes: size || null, status: 'pending'
  }).select('id,status,document_type,created_at').single();
