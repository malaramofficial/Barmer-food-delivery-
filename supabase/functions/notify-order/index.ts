...
Deno.serve(async req=>{
 ...
 if(![order.customer_id,order.restaurant_id,order.rider_id].filter(Boolean).includes(user.id)) return json({error:'Not allowed'},403);
 const recipients=[order.customer_id,order.rider_id].filter(Boolean);
 if(order.restaurant_id){ const {data:r}=await service.from('restaurants').select('owner_id').eq('id',order.restaurant_id).maybeSingle(); if(r?.owner_id) recipients.push(r.owner_id); }
 const unique=[...new Set(recipients)];
 if(unique.length) await service.from('notifications').insert(unique.map(user_id=>({user_id,order_id:body.order_id,title:String(body.title).slice(0,120),body:String(body.message).slice(0,500)})));
