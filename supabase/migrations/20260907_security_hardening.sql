-- Barmer Food Delivery security hardening migration
...
do $$ begin
  ...
  create policy "admin read restaurant applications" on public.restaurant_applications for select using (public.is_admin() or auth.uid()=applicant_id);
  create policy "admin read rider applications" on public.rider_applications for select using (public.is_admin() or auth.uid()=applicant_id);
exception when undefined_table then null; end $$;

-- Explicit admin read access for operational tables; writes remain Edge Function controlled.
do $$ begin
  drop policy if exists "admins read all orders" on public.orders;
  create policy "admins read all orders" on public.orders for select using (public.is_admin() or auth.uid()=customer_id or auth.uid()=rider_id or exists(select 1 from public.restaurants r where r.id=restaurant_id and r.owner_id=auth.uid()));
exception when undefined_table then null; end $$;

do $$ begin
  drop policy if exists "admins read all notifications" on public.notifications;
  create policy "admins read all notifications" on public.notifications for select using (public.is_admin() or auth.uid()=user_id);
exception when undefined_table then null; end $$;

-- Sensitive tables must never be publicly readable.
do $$ begin
  alter table public.rider_locations enable row level security;
  drop policy if exists "riders read own location" on public.rider_locations;
  create policy "riders read own location" on public.rider_locations for select using (public.is_admin() or auth.uid()=rider_id);
exception when undefined_table then null; end $$;

-- Useful indexes for abuse-resistant operational queries.
