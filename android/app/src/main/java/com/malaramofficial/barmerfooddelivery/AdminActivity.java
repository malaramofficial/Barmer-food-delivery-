...
/** Clean native operations portal. Backend role remains authoritative. */
public final class AdminActivity extends Activity {
    ...
    private void showLogin(){
        ...
        LinearLayout hero=new LinearLayout(this);hero.setOrientation(LinearLayout.VERTICAL);hero.setPadding(dp(20),dp(20),dp(20),dp(20));hero.setBackground(bg(DARK,20));TextView eyebrow=tv("BARMer FOOD • OPERATIONS",11,Color.rgb(205,205,210));bold(eyebrow,11);hero.addView(eyebrow);TextView title=tv("Staff portal",27,Color.WHITE);bold(title,27);title.setPadding(0,dp(5),0,0);hero.addView(title);hero.addView(tv("Secure access for administration and operations.",13,Color.rgb(220,220,224)));body.addView(hero);
        ...
        TextView note=tv("Only a backend account with role = admin can enter this area.",12,MUTED);note.setPadding(dp(2),dp(11),dp(2),dp(14));body.addView(note);
        Button login=button("Sign in");body.addView(login,new LinearLayout.LayoutParams(-1,dp(52)));login.setOnClickListener(v->{hideKeyboard();if(!api.configured()){toast("Backend is not configured in this build.");return;}String e=email.getText().toString().trim(),p=pass.getText().toString();if(e.isEmpty()||p.isEmpty()){toast("Email और password दोनों भरें");return;}login.setEnabled(false);api.signIn(e,p,new NativeApi.Callback(){public void ok(JSONObject d){verifyAdmin(login);}public void error(String m){runOnUiThread(()->{login.setEnabled(true);toast(m);});}});});
