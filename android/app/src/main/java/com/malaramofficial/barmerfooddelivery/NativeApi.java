...
public final class NativeApi {
    ...
    public void signIn(String email,String password,Callback cb){ auth("/auth/v1/token?grant_type=password",email,password,cb); }
    public void signUp(String email,String password,String name,String phone,Callback cb){
        JSONObject b=new JSONObject(); try{b.put("email",email);b.put("password",password);JSONObject d=new JSONObject();d.put("full_name",name);d.put("phone",phone);b.put("data",d);}catch(Exception e){cb.error(e.getMessage());return;}
        request("POST","/auth/v1/signup",b,false,new Callback(){public void ok(JSONObject d){saveSession(d);cb.ok(d);}public void error(String m){cb.error(m);}});
    }
    private void auth(String p,String e,String pw,Callback cb){JSONObject b=new JSONObject();try{b.put("email",e);b.put("password",pw);}catch(Exception x){cb.error(x.getMessage());return;}request("POST",p,b,false,new Callback(){public void ok(JSONObject d){saveSession(d);cb.ok(d);}public void error(String m){cb.error(m);}});}
