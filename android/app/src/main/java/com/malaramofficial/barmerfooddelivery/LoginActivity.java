...
/** Clean native authentication experience. Customer and staff entry points stay separate. */
public final class LoginActivity extends Activity {
    ...
    private LinearLayout body, fields; private NativeApi api; private Button action; private TextView loginTab, signupTab;
    private EditText email, password, name, phone;
    private boolean signupMode=false;

    @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(Color.WHITE);getWindow().setNavigationBarColor(Color.WHITE);getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);api=new NativeApi(this);build();}
    ...
    private void build(){
        ...
        Button google=btn("Continue with Google",Color.WHITE,INK);google.setBackground(outline(LINE,1,14));card.addView(google,new LinearLayout.LayoutParams(-1,dp(52)));google.setOnClickListener(v->{if(!api.configured()){showError(api.configurationStatus());return;}new GoogleAuth(this).signIn(this,api,new GoogleAuth.Callback(){public void ok(){openHome("Google login successful");}public void error(String m){showError(m);}});});
        ...
        if(!api.configured()){TextView warn=tv("Backend configuration is missing. Login and account creation are disabled until Supabase is configured.",12,Color.rgb(150,92,20));warn.setGravity(Gravity.CENTER);warn.setPadding(dp(6),dp(8),dp(6),0);body.addView(warn);action.setEnabled(false);google.setEnabled(false);}
        setMode(false);
    }
    ...
    private void submit(){hideKeyboard();String e=email.getText().toString().trim(),p=password.getText().toString();if(e.isEmpty()||p.isEmpty()){showError("Email और password दोनों भरें");return;}action.setEnabled(false);if(signupMode){api.signUp(e,p,name.getText().toString().trim(),phone.getText().toString().trim(),result("Account created"));}else{api.signIn(e,p,result("Login successful"));}}
    private NativeApi.Callback result(String success){return new NativeApi.Callback(){public void ok(JSONObject d){runOnUiThread(()->{action.setEnabled(true);openHome(success);});}public void error(String m){runOnUiThread(()->{action.setEnabled(true);showError(m);});}};}
